...
{
    public delegate bool CheckCorrect(double[] target, double[] output);
        ...
        public void TrainUntilDone(bool showEpochStats = false)
        {
            ...
            while (trainingAccuracy.Item1 != trainingAccuracy.Item2)
            {
                ...
                if (showEpochStats || trainingAccuracy.Item1 == trainingAccuracy.Item2)
                {
                    ...
                    WriteAccuracy(trainingAccuracy, "Training");
                    WriteAccuracy(testAccuracy, "    Test");
                    Wr("Training time: {0:hh\\:mm\\:ss}", (trainingEnd - trainingStart));
                    Wr("   Epoch time: {0:hh\\:mm\\:ss}", (DateTime.UtcNow - epochStart));
                    Wr("     Run time: {0:hh\\:mm\\:ss}", (DateTime.UtcNow - start));
                    Wr("================================");
                }

                net.Save("e" + EpochCount + ".json");
            }
            Wr("All done.");
            Wr("");
        }
